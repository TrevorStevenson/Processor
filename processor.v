...
module processor(
    ...
    input clock, reset;
	 ...
	 output [31:0] pc, fdl, dxl, xml, mwl;
	 output weT;
	 
	 assign fdl = decode_insn;
	 assign dxl = execute_insn;
	 assign xml = memory_insn;
	 assign mwl = writeback_insn;
    ...
    /* YOUR CODE STARTS HERE */
	 ...
	 wire [31:0] aluInA6, aluInA7;
	 assign aluInA6 = wx ? data_writeReg : execute_dataA;
	 assign aluInA7 = mx ? alu_out : aluInA6;
	 
	 wire [31:0] aluInB6, aluInB7;
	 assign aluInB6 = wxB ? data_writeReg : execute_dataB;
	 assign aluInB7 = mxB ? alu_out : aluInB6;
 	 
	 alu execute_alu(aluInA7, aluDataB, alu_sub, shamt2, aluResult, ine, ilt, eovf);
	 ...
	 assign aluDataB = muxB ? sx_immediate : aluInB7;
	 
	 // Bypassing
	 
	 wire mxB, wxB, mx, wx, dmemD;
	 bypass_control bp_contr(writeback_insn, memory_insn, execute_insn, writeback_we, mem_we, mxB, wxB, mx, wx, dmemD);
	 
	 wire [31:0] multDivResult;
	 wire multDivException, multDivReady;
	 
	 multdiv multiplierDivider(execute_dataA, execute_dataB, mult, div, clock, multDivResult, multDivException, multDivReady);
	 ...
	 assign data = dmemD ? data_writeReg : alu_in_B;
	 
	 assign rd_mem = memory_insn[26:22];
	 
	 wire [4:0] mem_op;
	 assign mem_op = memory_insn[31:27];
	 
	 wire lw_mem;
	 assign lw_mem = mem_op[3];
	 
	 wire is_alu_opm, t1m, t2m, t3m, t4m;
	 and andt1m(t1m, ~mem_op[4], ~mem_op[3], ~mem_op[2]);
	 and andt2m(t2m, ~mem_op[1], ~mem_op[0]);
	 and isALUopm(is_alu_opm, t1m, t2m);
	 and and101m(t3m, mem_op[2], ~mem_op[1], mem_op[0]);
	 and and011m(t4m, ~mem_op[2], mem_op[1], mem_op[0]);
	 
	 wire mem_we;
	 or orWem(mem_we, is_alu_opm, t3m, t4m, lw_mem);
